...
function Header() {
  const { loggedInUser: { username, avatarUrl } } = useContext(UserContext);
  ...
  return (
    <ul className="nav mischka-bckgrnd" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <div id="date-user-link" style={{ display: 'grid', alignItems: 'center', width: '15%' }}>
        ...
        <a href={username === 'Log in' ? '/' : '/user'} style={{ width: '30%', margin: 'auto' }}><img style={{ width: '100%' }} src={avatarUrl} alt="user icon" /></a>
